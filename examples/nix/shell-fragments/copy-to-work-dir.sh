...
#
# Requires: $PROJECT_NAME to be set by the calling derivation.
...
SRC_DIR="$PWD"
WORK_DIR=$(mktemp -d "/tmp/${PROJECT_NAME}-work-XXXXXX")
trap 'rm -rf "$WORK_DIR"' EXIT INT TERM

if [ ! -f "$SRC_DIR/pubspec.yaml" ] || [ ! -d "$SRC_DIR/lib" ]; then
  echo "ERROR: Run this from the examples/${PROJECT_NAME}/ directory." >&2
  exit 1
fi
